...
namespace YourDB {
    class Program {
            ...
            if (File.Exists(dir + "\\" + csv + ".csv")) {
                try {
                    ...
                    using (OleDbConnection conn = new OleDbConnection("Provider=Microsoft.Jet.OleDb.4.0; Data Source=" + dir + ";Extended Properties=\"Text;HDR=YES;FMT=Delimited\"")) {
                        conn.Open();
                        using (OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT * FROM [" + csv + ".csv]", conn)) {
                    ...
                    using (SQLiteConnection conn = new SQLiteConnection("Data Source=" + dir + "\\" + csv + ".sqlite;Version=3;")) {
                        conn.Open();
        ...
        static void usage() {
            ...
            Console.WriteLine("--csvfilename=Tilling OR -f=Tilling OR /csvfilename:Tilling OR /f:Tilling");
            Console.WriteLine("");
            Console.WriteLine("Only three parameters:");
            Console.WriteLine("--csvfilename, -f comma separated file with mutations table to read in; default: 'Tilling'");
            Console.WriteLine("--workingdirectory, -d path to csv file, DB will be created in the same directory with the same name; default: Environment.CurrentDirectory");
            Console.WriteLine("--addrecords, -r flag (!) whether to add records to existing table; default: create a new table");
            Console.WriteLine("");
            Console.WriteLine("Example:");
            Console.WriteLine(@"YourDB.exe --workingdirectory=C:\VS2015\WebTilling\DuWTill\App_Data\ --csvfilename=Tilling");
        }
    }
}
